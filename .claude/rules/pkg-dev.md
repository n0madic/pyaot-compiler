...
3. **Cross-module user classes are supported as return values.** A function like `def get(...) -> Response` from another module is valid — callers can read fields (`resp.status_code`) and call methods (`resp.ok()`) directly. The compiler round-trips `Type::Class` through `mir_merger::type_to_raw/raw_to_type`, offset-adjusting class ids and re-interning names per caller. **Limitation:** user-defined classes can't yet be referenced in type annotations across modules (e.g. `r: mymod.Response = ...`); annotations on cross-module values must be omitted or use stdlib types. Stdlib `RuntimeObject` types (`HTTPResponse`, `StructTime`) remain simpler because they don't depend on the cross-module path.
