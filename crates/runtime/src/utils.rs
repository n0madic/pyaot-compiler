...
/// Raise a ValueError exception, taking ownership of the message String.
/// Avoids the memory leak that occurs when longjmp skips Rust destructors.
///
/// # Safety
/// This function never returns (longjmp via rt_exc_raise_owned).
#[inline(never)]
pub unsafe fn raise_value_error_owned(mut msg: String) -> ! {
    let ptr = msg.as_mut_ptr();
    let len = msg.len();
    let cap = msg.capacity();
    std::mem::forget(msg);
    crate::exceptions::rt_exc_raise_owned(ExceptionType::ValueError as u8, ptr, len, cap)
}

/// Raise an IOError exception, taking ownership of the message String.
/// Avoids the memory leak that occurs when longjmp skips Rust destructors.
///
/// # Safety
/// This function never returns (longjmp via rt_exc_raise_owned).
#[inline(never)]
pub unsafe fn raise_io_error_owned(mut msg: String) -> ! {
    let ptr = msg.as_mut_ptr();
    let len = msg.len();
    let cap = msg.capacity();
    std::mem::forget(msg);
    crate::exceptions::rt_exc_raise_owned(ExceptionType::IOError as u8, ptr, len, cap)
}

/// Raise a RuntimeError exception, taking ownership of the message String.
/// Avoids the memory leak that occurs when longjmp skips Rust destructors.
///
/// # Safety
/// This function never returns (longjmp via rt_exc_raise_owned).
#[inline(never)]
pub unsafe fn raise_runtime_error_owned(mut msg: String) -> ! {
    let ptr = msg.as_mut_ptr();
    let len = msg.len();
    let cap = msg.capacity();
    std::mem::forget(msg);
    crate::exceptions::rt_exc_raise_owned(ExceptionType::RuntimeError as u8, ptr, len, cap)
}
