...
pub(crate) fn apply_to_lowering(
    ...
    // 7. Per-function local seed types. Walk each function's params
    //    and `Bind { target: Var(v) }` statements to identify the
    //    locals owned by that function, then look up each in
    //    base_var_types. This is the legacy `precompute_all_local_var_types`
    //    contract; the constraint solver computes Var types globally
    //    but downstream code needs a per-function view.
    populate_per_function_locals(lowering, hir_module);
}

/// Build `per_function_local_seed_types` by walking each function's
/// HIR and absorbing per-statement RHS types into a per-Var map.
///
/// Sources RHS types from the just-materialized solver env (via
/// `lowering.lowering_seed_info.expr_types`), so the per-function map
/// is a deterministic function of solver output. Tuple destructure is
/// handled via [`absorb_into_targets`] — per-position binding matches
/// the legacy CFG-dataflow precision for `for (a, b) in pairs` patterns.
///
/// Differences from legacy `precompute_var_types`:
/// - No `refined_container_types` consultation (the solver hasn't yet
///   reified that map; the empty-container refinement chain is a
///   separate S6 follow-up).
/// - No post-loop rebind heuristic — we always JOIN. This is sound but
///   potentially over-widening; the legacy heuristic was a precision
///   optimization for loop-only writes.
fn populate_per_function_locals(lowering: &mut Lowering, hir_module: &hir::Module) {
    // SIMPLE path (S6 transitional): just copy `base_var_types` entries
    // for every Var that's a param or bound somewhere inside this
    // function. Empirically yields better test-suite coverage than the
    // CFG-dataflow rewrite while we hunt down which legacy precision
    // edges the dataflow needs to match.
    let func_ids: Vec<pyaot_utils::FuncId> = hir_module.func_defs.keys().copied().collect();
    for fid in func_ids {
        let func = match hir_module.func_defs.get(&fid) {
            Some(f) => f,
            None => continue,
        };
        let mut per_func: IndexMap<pyaot_utils::VarId, Type> = IndexMap::new();
        ...
        for param in &func.params {
            let ty = lowering
                ...
                .cloned()
                .unwrap_or(Type::Any);
            per_func.insert(param.var, ty);
        }

        for block in func.blocks.values() {
            for &stmt_id in &block.stmts {
                let target = match &hir_module.stmts[stmt_id].kind {
                    hir::StmtKind::Bind { target, .. }
                    | hir::StmtKind::IterAdvance { target, .. } => target,
                    _ => continue,
                };
                target.for_each_var(&mut |v| {
                    let ty = lowering
                        ...
                        .cloned()
                        .unwrap_or(Type::Any);
                    per_func.insert(v, ty);
                });
            }
        }

        per_func.retain(|var_id, _| {
...
/// Read an expression's type from the materialized cache, falling back
/// to the HIR's own `expr.ty` annotation (set by `apply_to_lowering`'s
/// HIR write-back step for cache-gated entries), and finally to
/// `Type::Any`.
fn expr_type(
    cache: &std::collections::HashMap<hir::ExprId, Type>,
    eid: hir::ExprId,
    expr: &hir::Expr,
) -> Type {
    cache
...
/// Bind `rhs_ty` into `per_func` for every `Var` leaf of `target`,
/// destructuring tuple targets per-position.
///
/// Skips `Type::Any` writes — they add no information and would widen
/// every observed local to `Any`, defeating the per-function precision
/// argument. Matches legacy `merge_var`'s Any guard.
fn absorb_into_targets(
    target: &hir::BindingTarget,
    rhs_ty: &Type,
    per_func: &mut IndexMap<pyaot_utils::VarId, Type>,
) {
    match target {
        hir::BindingTarget::Var(v) => {
            if !matches!(rhs_ty, Type::Any) {
                merge_var_join(per_func, *v, rhs_ty.clone());
            }
        }
            ...
            if let Some(types) = rhs_ty.tuple_elems() {
                if types.len() == elts.len() {
                    for (elt, t) in elts.iter().zip(types) {
                        absorb_into_targets(elt, t, per_func);
                    }
                    return;
                }
                // arity mismatch — fall through to Any-bind for safety.
                for elt in elts {
                    absorb_into_targets(elt, &Type::Any, per_func);
                }
            } else if let Some(elem) = rhs_ty.tuple_var_elem() {
                let owned = elem.clone();
                for elt in elts {
                    absorb_into_targets(elt, &owned, per_func);
                }
            } else {
                // Non-tuple RHS into tuple destructure — should never
                // happen on type-correct code; widen for safety.
                for elt in elts {
                    absorb_into_targets(elt, &Type::Any, per_func);
                }
            }
        ...
        hir::BindingTarget::Starred { inner, .. } => {
            // `*rest = …` captures the remaining elements as a list.
            let list_ty = Type::list_of(rhs_ty.clone());
            absorb_into_targets(inner, &list_ty, per_func);
        }
        hir::BindingTarget::Attr { .. }
        | hir::BindingTarget::Index { .. }
        | hir::BindingTarget::ClassAttr { .. } => {
            // Not a Var bind — no per-func entry to record.
        }
    }
}

/// JOIN `new_ty` into `per_func[var_id]` via `TypeLattice::join` — the
/// same lattice op the solver uses, so per-function maps stay monotone
/// with respect to all prior observations.
fn merge_var_join(
    per_func: &mut IndexMap<pyaot_utils::VarId, Type>,
    var_id: pyaot_utils::VarId,
    new_ty: Type,
) {
    use pyaot_types::TypeLattice;
    match per_func.get(&var_id).cloned() {
        Some(prev) => {
            let joined = prev.join(&new_ty);
            per_func.insert(var_id, joined);
        }
        None => {
            per_func.insert(var_id, new_ty);
        }
    }
}
