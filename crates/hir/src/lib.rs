...
pub enum ExprKind {
    ...
    // ==================== Format ====================
    /// f-string format spec: `f"{value:spec}"` → `format(value, "spec")` via runtime dispatch.
    /// The `spec` is the raw spec string (e.g. `"4d"`, `".4f"`, `">10.2f"`).
    /// Lowering calls `RT_FORMAT(boxed_value, spec_str)`.
    FormatSpec {
        value: ExprId,
        spec: InternedString,
        span: Span,
