...
impl AstToHir {
    ...
    fn convert_call_expr(&mut self, call: py::ExprCall, expr_span: Span) -> Result<ExprId> {
    ...
    fn convert_name_call(
        ...
        if let Some(&func_id) = self.func_map.get(&name_str) {
        ...
        // Check if it's an imported name
        if let Some(imported) = self.imported_names.get(&name_str).cloned() {
            let func_expr = match imported.kind {
                super::ImportedNameKind::Function(func_id) => self.module.exprs.alloc(Expr {
                    kind: ExprKind::FuncRef(func_id),
                    ty: None,
                    span: expr_span,
                }),
                super::ImportedNameKind::Class(class_id) => self.module.exprs.alloc(Expr {
                    kind: ExprKind::ClassRef(class_id),
                    ty: None,
                    span: expr_span,
                }),
