# Phase 8C — File I/O core (focused fixture): open/read/readline/readlines/
# write/close, the `with` context manager, append/r+/w+/a+ modes, read(n),
# encoding=, file iteration + comprehension, str methods, io.StringIO/BytesIO,
# and ValueError on a bad mode. The non-UTF-8 binary-read block from
# test_file_io.py is excluded (it needs non-UTF-8 bytes literals — an
# interner/parser feature outside File I/O). Self-checking: temp /tmp paths.
