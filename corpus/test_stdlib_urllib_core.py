# Phase 8D — urllib (core, deterministic): urllib.parse (urlparse/quote/
# unquote/urljoin/urlencode/parse_qs + ParseResult fields/geturl), Request
# construction + fields, and the urllib.error exception hierarchy
# (HTTPError/URLError caught by their own name, by OSError, and by Exception).
# The network urlopen/urlretrieve sections of test_stdlib_urllib.py are excluded
# (non-deterministic + require connectivity; PYAOT_NET_TESTS territory).
