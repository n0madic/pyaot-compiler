...
| §1.11 Deferred HIR-tree deletion | 🟡 | S1.17b-a ✅ · S1.17b-b ✅ · S1.17b-e ✅ · S1.17b-d partial (ni_analysis, collect_handler_binds) · S1.17b-c / S1.17b-f ⏳ |
...
| S1.17b-d 🟡 (partial 2026-04-19) | Walkers + generator desugar (§1.11 Stage 4). **Landed**: `ni_analysis.rs` iterates CFG blocks; `collect_handler_binds` reads `try_scopes` directly. **Still tree-walking**: `container_refine.rs` (needs successor-chain walk with merge-point reconciliation), `closure_scan.rs`, `local_prescan.rs`, `lambda_inference.rs`, `collect_return_types` (acceptable per-function-flat walk), `generators/desugaring.rs` (tree synthesis + bridge-via-build_cfg_from_tree works fine today). Deletion gate (S1.17b-f) blocked until these migrate. | S1.17b-c | **HIGH** | — |
