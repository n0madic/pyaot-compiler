...
**Documented dynamic-arg gap:** a join argument typed `Any` / `Union` / `Iterator` (generator) has no concrete container type at lowering, so no converter can be chosen — `snapshot_iterable_to_list` returns the operand unchanged and the arg stays unsupported (same status as today, mirroring the deferred deque-parameter gap). Separately, `list[bytes]` element equality / `sorted()` is a pre-existing unrelated gap (scalar `bytes` comparison works); the `bytes.join`-over-set test verifies via the decoded-`str` sorted trick to avoid it.
