...
- `devirt_indirect::devirt_wrapper_indirect_calls` rewrites `rt_call_with_tuple_args` / `rt_call_with_captures_and_args` runtime trampoline sites in the spec's body to `CallDirect{captured_id, [user_params]}` — but only when the wrapper's user-visible signature matches the captured target slot-for-slot. `*args` wrappers (where the user-visible param is a packed `tuple[...]` while the captured target wants N individual args) don't match: devirt is skipped, runtime trampoline keeps them correct, and the spec's return type stays `Any`. A future Stage can teach devirt to emit unpack instructions to bridge the gap.
