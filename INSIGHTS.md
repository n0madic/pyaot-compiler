...
**Class-method monomorphization (S3.3b.1)**: Generic class instances are typed `Type::Generic{class_id, [concrete_args]}` end-to-end. The frontend captures TypeVar parameters into `ClassDef.type_params` (both `Generic[T]` bases and PEP-695 `class C[T]:`). Method bodies pre-stamp `self: Generic{class_id, [Var(T)..]}` so methods using `T` mark `is_generic_template`. Lowering's `lower_method_call` dispatches `Type::Generic{base, ..}` receivers to `lower_class_method_call` (mirror of the `Type::Class` arm); attribute reads/writes (`lower_attribute`, `bind_attr_op`) extract `class_id` from both `Type::Class` and `Type::Generic`. After the constructor lowers, `lower_assign` promotes the alloc-result local from `Class{id}` to the explicit `Generic{id, args}` hint to prevent WPA from collapsing the type-args. Constructor `__init__` specializes via the standard `CallDirect` template path. Methods using `T` (like `transform`) work via the un-specialized template because `CallVirtual` codegen builds per-call signatures dynamically, so pass-through values flow correctly. Methods with concrete signatures (like `unwrap(self) -> int`) compile as plain class methods. **Pre-mono devirt was rejected**: converting `CallVirtual` → `CallDirect(template)` before `monomorphize::run` broke `__exit__(self, *a)` and other vararg methods because the lowering does not pack varargs at method-call sites — `CallVirtual` codegen tolerates the arity gap (dynamic per-call signature), but `CallDirect` requires the strict callee ABI. The vtable purge guard in `monomorphize_module` (`vtable_refs: HashSet<FuncId>`) is still required: generic-class method templates referenced from vtables but not from any `CallDirect` would otherwise be purged, leaving dangling vtable slots.
