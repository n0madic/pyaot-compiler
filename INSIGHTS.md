...
**Class-method monomorphization deferred**: Generic class fields and method annotations that use `T` will type-erase via the existing TypeVar mechanism (unconstrained `T` → `Type::Var` → WPA may infer, or emit type-mismatch diagnostic). Class-method specialization (S3.3b) is a separate session.
