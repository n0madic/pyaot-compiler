...
This document contains internal implementation details for developers working on the compiler.
...
- **HIR**:
  ...
  - `StmtKind::ForUnpackStarred { before_star, starred, after_star, iter, body, else_block }`
- **Lowering**:
...
- **Runtime** (`exceptions.rs`):
  - `ExceptionFrame`: prev (8) + jmp_buf (200 bytes) + gc_stack_top (8) + traceback_depth (8) = 224 bytes
  - `ExceptionObject`: exc_type, custom_class_id, message, message_len, message_capacity, cause (`__cause__`), context (`__context__`), suppress_context, traceback (Option<Vec<TracebackEntry>>), instance (heap-allocated exception object)
...
- **Runtime** (`object.rs`):
  - `TypeTag::Iterator = 9`
  - `IteratorKind`: List=0, Tuple=1, Dict=2, String=3, Range=4, Set=5, Bytes=6, Enumerate=7, Zip=8, Map=9, Filter=10, Chain=11, ISlice=12, Zip3=13, ZipN=14
...
- **Lowering**: `box_primitive_if_needed()` helper boxes primitives during assignment
...
- `crates/lowering/src/lib.rs`: `box_primitive_if_needed()` helper
